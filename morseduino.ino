/* 
   Dave Mueller W9BFZ's modification of:
...
int myKey=3;    // We are borrowing Analog Pin 0 and using it as digital
int speaker=4;  // Speaker will be hooked between pin 11 and ground

int val=0;                   // A value for key up and down
int myTone=0;              // Frequency of our tone
int tonePot = A1; // Tone poteniometer
int pot = 0; // Control the frequency of the side tone. 
...
void setup() {
 ...
 void loop() {
  pot = analogRead(tonePot); // Read tone potentiometer
  myTone = map(pot, 0, 1023, 350, 900); // Set tone based on potentiometer. Between 400hz and 900hz
